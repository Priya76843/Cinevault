...
const MovieModal = ({ movie, onClose }) => {
  ...
  // Opens YouTube Trailer in new tab (assignment requirement)
        ...
        <div className="flex flex-col md:flex-row gap-8 px-10 pb-10 pt-2 overflow-y-auto no-scrollbar">
          ...
          <div className="w-full md:w-[200px] flex-shrink-0 flex flex-col gap-4">
            ...
            <button 
              ...
              className="w-full bg-transparent border border-neutral-700 text-white font-semibold text-[13px] py-2.5 rounded hover:bg-neutral-800 transition-colors cursor-pointer"
            >
              + Watchlist
            </button>
          </div>
          ...
          <div className="flex-1 flex flex-col pt-2">
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              {movie.genres?.map(genre => (
                <span key={genre} className="bg-[#1a1a24] text-neutral-300 text-[11px] px-2.5 py-1 rounded font-medium">
                  {genre}
                </span>
              ))}
            </div>
